...
function Question(props) {
  ...
  useEffect(() => {
    for (let i = 0; i < length; i++) {
      ...
      res.results[ques].incorrect_answers = res.results[ques].incorrect_answers.map((x) =>
  ...
  useEffect(() => {
    ...
    setoptions([res.results[ques].correct_answer, ...res.results[ques].incorrect_answers]);
