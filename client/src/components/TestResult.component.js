...
function Testresult(props) {
  ...
  useEffect(() => {
    ...
    axios
      .post(`${url}/api/test/getresults`, { pin: props.location.state.pin }, options)
    ...
    <Fragment>
      ...
      <div className={teststyles.container}>
        <div className={resultstyles.info}>
          ...
          <strong>Expiry: </strong> {expiry.getDate()}-{expiry.getMonth()}-{expiry.getFullYear()}
          <br />
        </div>
        <div className={resultstyles.parent}>
          ...
          {result.length === 0 ? (
            <div className={resultstyles.resultrow}>
              <div className={teststyles.element} style={{ gridColumnStart: "2" }}>
                No result found!
              </div>
            </div>
          ) : (
